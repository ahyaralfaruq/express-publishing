...
describe("Artist Table", function () {
   ...
   it("should have name, date_of_birth, biography, and is_currently_employed columns with appropriate data types", function (done) {
      prodDb.run(
         "INSERT INTO Artist (name, date_of_birth, biography, is_currently_employed) VALUES ('Artist Name', 'January 1, 1980', 'My Biography', 1)",
         function (error) {
            if (error) {
               done(new Error(error));
            } else {
               prodDb.run(
                  `DELETE FROM Artist WHERE Artist.id = ${this.lastID}`,
                  () => {
   ...
   it("should have a required name column", function (done) {
      prodDb.run(
         "INSERT INTO Artist (date_of_birth, biography, is_currently_employed) VALUES ('January 1, 1980', 'My Biography', 1)",
         function (error) {
            if (
               error &&
   ...
   it("should have a required date_of_birth column", function (done) {
      prodDb.run(
         "INSERT INTO Artist (name, biography, is_currently_employed) VALUES ('Artist Name', 'My Biography', 1)",
         function (error) {
            if (
               error &&
               error.toString().includes("NOT NULL constraint failed")
               ...
               prodDb.run(
                  ...
                  () => {
                        ...
                        new Error("Artist without date_of_birth was created.")
                     );
                  }
               );
            } else {
   ...
   it("should have a required biography column", function (done) {
      prodDb.run(
         "INSERT INTO Artist (name, date_of_birth, is_currently_employed) VALUES ('Artist Name', 'January 1, 1980', 1)",
         function (error) {
            if (
               error &&
               error.toString().includes("NOT NULL constraint failed")
   ...
   it("is_currently_employed should default to 1", function (done) {
      prodDb.run(
         "INSERT INTO Artist (name, date_of_birth, biography) VALUES ('Artist Name', 'January 1, 1980', 'My Biography')",
         function (error) {
            if (error) {
               done(new Error(error));
            } else {
               ...
               prodDb.get(
                  ...
                  (error, artist) => {
                     prodDb.run(
                        `DELETE FROM Artist WHERE Artist.id = ${artistId}`,
                        () => {
...
describe("Series Table", function () {
   ...
   it("should have id, name, and description columns with appropriate data types", function (done) {
      prodDb.run(
         "INSERT INTO Series (name, description) VALUES ('Series Name', 'Series Description')",
         function (error) {
            if (error) {
               done(new Error(error));
            } else {
               prodDb.run(
                  `DELETE FROM Series WHERE Series.id = ${this.lastID}`,
                  () => {
                     expect(this.lastID).to.exist;
                     done();
                  }
   ...
   it("should have a required name column", function (done) {
      prodDb.run(
         "INSERT INTO Series (description) VALUES ('Series Description')",
         function (error) {
            if (
               error &&
               error.toString().includes("NOT NULL constraint failed")
   ...
   it("should have a required description column", function (done) {
      prodDb.run(
         "INSERT INTO Series (name) VALUES ('Series Name')",
         function (error) {
            if (
               error &&
               error.toString().includes("NOT NULL constraint failed")
...
describe("Issue Table", function () {
   ...
   it("should have id, name, issue_number, publication_date, artist_id, and series_id columns with appropriate data types", function (done) {
      prodDb.run(
         "INSERT INTO Issue (name, issue_number, publication_date, artist_id, series_id) VALUES ('Issue Name', 1, 'January 1, 1980', 1, 1)",
         function (error) {
            if (error) {
               done(new Error(error));
            } else {
               prodDb.run(
                  `DELETE FROM Issue WHERE Issue.id = ${this.lastID}`,
                  () => {
                     expect(this.lastID).to.exist;
                     done();
                  }
   ...
   it("should have a required name column", function (done) {
      prodDb.run(
         "INSERT INTO Issue (issue_number, publication_date, artist_id, series_id) VALUES (1, 'January 1, 1980', 1, 1)",
         function (error) {
            if (
               error &&
               error.toString().includes("NOT NULL constraint failed")
   ...
   it("should have a required issue_number column", function (done) {
      prodDb.run(
         "INSERT INTO Issue (name, publication_date, artist_id, series_id) VALUES ('Issue Name', 'January 1, 1980', 1, 1)",
         function (error) {
            if (
               error &&
               error.toString().includes("NOT NULL constraint failed")
   ...
   it("should have a required publication_date column", function (done) {
      prodDb.run(
         "INSERT INTO Issue (name, issue_number, artist_id, series_id) VALUES ('Issue Name', 1, 1, 1)",
         function (error) {
            if (
               error &&
               error.toString().includes("NOT NULL constraint failed")
               ...
               prodDb.run(
                  ...
                  () => {
                        ...
                        new Error("Issue without publication_date was created.")
                     );
                  }
               );
            } else {
   ...
   it("should have a required artist_id column", function (done) {
      prodDb.run(
         "INSERT INTO Issue (name, issue_number, publication_date, series_id) VALUES ('Issue Name', 1, 'January 1, 1980', 1)",
         function (error) {
            if (
               error &&
               error.toString().includes("NOT NULL constraint failed")
   ...
   it("should have a required series_id column", function (done) {
      prodDb.run(
         "INSERT INTO Issue (name, issue_number, publication_date, artist_id) VALUES ('Issue Name', 1, 'January 1, 1980', 1)",
         function (error) {
            if (
               error &&
               error.toString().includes("NOT NULL constraint failed")
...
describe("GET /api/artists", function () {
   ...
   it("should return a status code of 200", function () {
      return request(app).get("/api/artists").expect(200);
   });
});
...
describe("GET /api/artists/:id", function () {
   ...
   it("should return a 200 status code for valid IDs", function () {
      return request(app).get("/api/artists/2").expect(200);
   });

   it("should return a 404 status code for invalid IDs", function () {
      return request(app).get("/api/artists/999").expect(404);
   });
});
...
describe("PUT /api/artists/:id", function () {
   ...
   it("should update the artist with the given ID", function (done) {
      request(app)
         ...
         .then(function () {
            testDb.get(
               "SELECT * FROM Artist WHERE Artist.id = 1",
               function (error, artist) {
                  ...
                  expect(artist.date_of_birth).to.equal(
                     updatedArtist.dateOfBirth
                  );
                  expect(artist.biography).to.equal(updatedArtist.biography);
                  expect(artist.is_currently_employed).to.equal(
                     updatedArtist.isCurrentlyEmployed
                  );
                  done();
               }
            );
...
describe("DELETE /api/artists/:id", function () {
   ...
   it("should return a 200 status code after artist delete", function () {
      return request(app).del("/api/artists/1").expect(200);
   });
...
describe("GET /api/series", function () {
   ...
   it("should return a status code of 200", function () {
      return request(app).get("/api/series").expect(200);
   });
});
...
describe("GET /api/series/:id", function () {
   ...
   it("should return a 200 status code for valid IDs", function () {
      return request(app).get("/api/series/2").expect(200);
   });

   it("should return a 404 status code for invalid IDs", function () {
      return request(app).get("/api/series/999").expect(404);
   });
});
         ...
         .then(function () {
            testDb.all("SELECT * FROM Series", function (error, result) {
               ...
               const series = result.find(
                  (series) => series.name === newSeries.name
               );
         ...
         .then(function () {
            testDb.get(
               ...
               function (error, series) {
                  ...
                  expect(series.description).to.equal(
...
describe("DELETE /api/series/:id", function () {
   ...
   it("should return a 204 status code after series delete", function () {
      return request(app).del("/api/series/1").expect(204);
   });
   ...
   it("should not delete series with existing related issues", function () {
      return request(app)
         ...
         .then(function () {
            testDb.get(
               "SELECT * FROM Series WHERE Series.id = 2",
               function (error, series) {
                  if (error) {
                     throw new Error(error);
                  }
   ...
   it("should return a 400 status code if deleted series has existing related issues", function () {
      return request(app).del("/api/series/2").expect(400);
   });
});
...
describe("GET /api/series/:seriesId/issues", function () {
   ...
   it("should return a status code of 200 for valid series", function () {
      return request(app).get("/api/series/2/issues").expect(200);
   });

   it("should return a status code of 404 for invalid series", function () {
      return request(app).get("/api/series/999/issues").expect(404);
   });
});
         ...
         .then(function () {
            testDb.all("SELECT * FROM Issue", function (error, result) {
               ...
               const issue = result.find(
                  (issue) => issue.name === newIssue.name
               );
               ...
               expect(issue.publication_date).to.equal(
...
describe("PUT /api/series/:seriesId/issues/:issueId", function () {
   ...
   it("should update the issue with the given ID", function (done) {
      request(app)
         ...
         .then(function () {
            testDb.get(
               "SELECT * FROM Issue WHERE Issue.id = 1",
...
describe("DELETE /api/series/:seriesId/issues/:issueId", function () {
   ...
   it("should return a 204 status code after issue delete", function () {
      return request(app).del("/api/series/2/issues/1").expect(204);
   });

   it("should return a 404 status code for invalid issue IDs", function () {
      return request(app).del("/api/series/2/issues/999").expect(404);
   });
});
