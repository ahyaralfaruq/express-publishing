   ...
   (error, table) => {
      ...
      if (table) {
         db.serialize(function () {
            db.run(
               "INSERT INTO Artist (name, date_of_birth, biography) VALUES ('Stan Lee', 'December 28, 1922', 'I definitely work here')"
            );
            db.run(
               "INSERT INTO Artist (name, date_of_birth, biography) VALUES ('Jack Kirby', 'August 28, 1917', 'I also definitely work here')",
               function (error) {
                  if (error) {
                     throw new Error(error);
                  }
            ...
            db.get(
               ...
               (error, table) => {
                  ...
                  if (table) {
                     db.serialize(function () {
                        db.run(
                           "INSERT INTO Series (name, description) VALUES ('Pyder Man', 'A web-slinging snake slithers through Queens cleaning the streets of bad data')"
                        );
                        db.run(
                           "INSERT INTO Series (name, description) VALUES ('BashMan', 'An orphaned superhero learns the power of taking commands from others and performing them exactly as expected')",
                           function (error) {
                              if (error) {
                                 throw new Error(error);
                              }
